var bot = {
    ...
    text: {
        ...
        insert(){       
            switch(this.template){
                case 'parrot': {
                    const logKids = bot.log.children
                    const strangerMsg = logKids[logKids.length-4].firstChild.textContent;
                    const msg = strangerMsg.replace(/Stranger:\s/, '');
                    console.log(msg);
    ...
    start(){
        ...
        this.botInterval = setInterval( () => {
            ...
            if(this.text.isReplyMode){
                try {
                    const logKids = bot.log.children
                    const strangerMsgDiv = logKids[logKids.length-4].firstChild;
                    console.log(strangerMsgDiv);

                    if(strangerMsgDiv.classList.contains('strangermsg') || (this.text.isReplyAll && this.text.counter > 1)){
                        this.text.insert();
                        this.sendMsg();
                    }
                } catch(err){}
    ...
    leaveIfDisconnected(){
        if(this.btn.disabled && this.isAutoNext){
            this.btnEsc.click();
            this.text.reset();
        }
    },
    ...
    init(inputQuery, btnQuery = 0, btnEscQuery = 0, logQuery = 0){
        ...
        this.log = logQuery ? document.querySelector(logQuery) : null;
...
//6obcy
bot.init('.chatmsg', '.sendbtn', '.disconnectbtn', '.logbox > div');
